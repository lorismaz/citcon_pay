...
VCR.configure do |config|
  ...
  config.filter_sensitive_data('<API_KEY>') { ENV['CITCON_API_KEY'] || 'test-api-key' }
  config.filter_sensitive_data('<ACCESS_TOKEN>') do |interaction|
    # Filter from Authorization header in requests
    auth_header = interaction.request.headers['Authorization']&.first
    auth_header.gsub(/^Bearer /, '') if auth_header && auth_header.start_with?('Bearer ')
  end
  config.filter_sensitive_data('<ACCESS_TOKEN>') do |interaction|
    # Filter from response bodies
...
RSpec.configure do |config|
  ...
  # Allow WebMock stubs for unit tests (those tagged with :stub)
  # VCR cassettes will be used for integration tests
  config.around(:each, :stub) do |example|
...
module SpecHelpers
  ...
  def stub_charge_request(charge_data: {}, response_data: {})
    ...
    stub = stub.with(body: charge_data.to_json) unless charge_data.empty?
  ...
  def stub_transaction_request(transaction_id:, transaction_data: {})
    ...
    # Manually merge nested data
    merged_data = default_data.dup
    merged_data[:data] = default_data[:data].merge(transaction_data[:data]) if transaction_data[:data]
    merged_data.merge!(transaction_data.reject { |k, _| k == :data })
